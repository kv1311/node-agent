...
router.get('/finance/transactions', async (req, res) => {
  try {
    const { limit = 50, month } = req.query
...
// Monthly rollups — last 6 months of history
router.get('/finance/rollups', async (req, res) => {
  try {
    const result = await db.execute(
      `SELECT month, total_inflow, total_outflow, uncle_investment_total, last_updated
