...
/**
 * Validate obligation parameters
 */
function validateObligation(args) {
  const errors = []

  if (!args.from || !args.to) {
    errors.push('Missing from or to party')
  }
  ...
  if (args.from === args.to) {
    errors.push('Cannot create obligation where from and to are the same')
  }
...
/**
 * CREATE OBLIGATION
 * Generates obligation_id, stores in DB and memory
 */
export async function createObligation(args) {
  try {
    ...
    if (installments > 1) {
      label += ` in ${installments} installments`
    }
...
/**
 * RECORD SETTLEMENT
 * Partial or full payment towards an obligation
 */
export async function recordSettlement(args) {
  try {
    ...
    if (!args.obligation_id || !args.amount_paid) {
      return {
        status: 'Failed',
        error: 'Missing obligation_id or amount_paid',
      }
    }
    ...
    if (!result.rows || !result.rows.length) {
      return {
        status: 'Failed',
        error: `Obligation ${args.obligation_id} not found`,
      }
    }
    ...
    // Record settlement in audit trail
    ...
    // Update memory
    const newLabel = `${oblig.from_party} owes ${oblig.to_party} ₹${newRemaining}${
      isFullyPaid ? ' [SETTLED]' : ''
    }`
    await upsertMemoryNode({
      canonical_key: `obligation:${args.obligation_id}`,
      label: newLabel,
      type: 'finance',
      metadata: {
        obligation_id: args.obligation_id,
        from: oblig.from_party,
        to: oblig.to_party,
        total_amount: oblig.total_amount,
        paid_total: newPaidTotal,
        remaining: newRemaining,
        currency: oblig.currency,
        due_date: oblig.due_date,
        installments: oblig.installments,
        purpose: oblig.purpose,
...
/**
 * QUERY OBLIGATIONS
 * Flexible querying of obligations
 */
...
/**
 * UPDATE BALANCE
 * Store current account balance (not an obligation)
 */
export async function updateBalance(args) {
  try {
    ...
    await upsertMemoryNode({
      canonical_key: `balance:${account.toLowerCase()}`,
      label: `${account} balance: ₹${amount.toFixed(2)}`,
      type: 'finance',
      metadata: {
        account,
        balance: amount,
    ...
    return {
      status: 'Success',
      details: `${account} balance updated to ₹${amount.toFixed(2)}`,
    }
  } catch (error) {
 ...
 * Fetch full history of a single obligation with all settlements
 */
  ...
  try {
    ...
    return {
      ...
      obligation: {
        ...
        from: oblig.from_party,
        to: oblig.to_party,
        total: oblig.total_amount,
        paid: oblig.paid_total,
        remaining: oblig.remaining,
        due: oblig.due_date,
        installments: oblig.installments,
        purpose: oblig.purpose,
        status: oblig.status,
        created: oblig.created_at,
        lastPayment: oblig.last_payment_date,
      },
